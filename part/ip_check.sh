...
declare UA_Browser
declare rawgithub
declare ad_base="https://raw.githubusercontent.com/jyogyou/NodeQuality/refs/heads/main/"
...
while :;do
local content
content=$(curl -fsL --max-time 5 "${ad_base}ref/ad$i.ans")||break
ads+=("$content")
((i++))
done
ADLines=0
...
local -a aad
aad[0]=$(curl -fsL --max-time 5 "${ad_base}ref/sponsor.ans") || true
[[ -z "${aad[0]}" ]]&&return
